...
class HttpServer
{
    public:
        ...
        HttpServer(EventLoop*loop,std::string ip,int port,int pollSize,int mode)
                :server_(loop,ip,port,pollSize,mode),sql_pool(connection_pool::GetInstance())
        {
            server_.setConnectionCallback(std::bind(&HttpServer::onConnection,this,_1));
            server_.setMessageCallback(std::bind(&HttpServer::onMessage,this,_1,_2));
        }
        void sql_pool_init(string url, string User, string PassWord, string DBName, int Port, int MaxConn, int close_log);
